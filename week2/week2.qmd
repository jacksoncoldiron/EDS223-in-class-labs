...
```{r}
world_df <- world |>
  select(-geom) # Doesn't get rid of the geom column
...
class(world_df) # No sf class associated with it now, solely a df
```
...
```{r}
# Create a point for London in a projected coordinate system
...
```{r}
# Transform the CRS of the London sf object to match the projected London point
...
```{r}
# Building a CRS check into the workflow
...
```{r}
# Filter to countries less than 10,000 km2
world1 <- world |>
...
n_countries <- world |>
  group_by(continent) |>
  summarize(population = sum(pop, n.rm = TRUE),
