...
# test the class of raster object
class(zion_elevation) # spatial raster (SpatRaster)
class(zion_land)
```

```{r}
map1 <- tm_shape(zion_elevation) +
  tm_raster(col.legend = tm_legend("Electation (m)"))
...
nlyr(landsat) # 4 layers (bands)

# Find out the names of the layers
names(landsat)
...
map3 <- tm_shape(merged) +
  tm_raster(col.legend = tm_legend("Elevation (m)")) +
  tm_title(text = "Austria & Switzerland")
...
# Assign elevation of less than 20 to an NA
...
# Approach 1: find Cell ID for that point
...
terra::ext(zion_elevation) # bounding box of current raster

# Create raster with a smaller extent 
clip <- rast(xmin = -113.3, xmax = -113, ymin = 37.2, ymax = 37.9,
             resolution = 0.3, # 0.3 x 0.3 degree pixels
             vals = 1) 

# Select the values that fall within smaller extent
zion_elevation_clip <- zion_elevation[clip]

# Verify the output has fewer values than original 
...
```{r}
zion_elevation_clip <- zion_elevation[clip, drop = FALSE] # don't drop the geometries associated with the raster
...
# We want our extents not to match
...
```{r}
# Create a raster mask (duplicate of Zion)
rmask <- zion_elevation
     
# Set all the cells that have elevation less than 2000m to NA
rmask[rmask < 2000] <- NA

# Subset elevation raster based on the mask
...
map2 <- tm_shape(masked1) +
  tm_raster(col.legend = tm_legend(title = "Elevation (m)")) +
  tm_title(text = "Bracker Subsettiing")
...
```{r}
zion_elevation + zion_elevation # double each cell value

zion_elevation^2 # square each value of the cell

log(zion_elevation) # log of each cell
...
rcl <- matrix(c(
  1000, 1500, 1, # group 1 ranges from 1000 - 1500 m
  1500, 2000, 2, # group 2
  2000, 2500, 3, # group 3
  2500, 3000, 4 # group 4
),
...
# Change reclassify values into factors 
...
# Apply NDVI functino to landsat bands 3 and 4 (red and NIR)
...
```{r}
# Adding two rows to the top and bottom 200 to the right and the left
...
```{r}
zion_elevation + elev_2 # Different extents don't work for algebraic operations
```
...
zion_elevatioin_disagg <- disagg(zion_elevation_coarse, fact = 5, method = "bilinear")
